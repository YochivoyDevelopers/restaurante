...
export class AddNewFoodsPage implements OnInit {
    ...
    const actionSheet = await this.actionSheetController.create({
      ...
      buttons: [{
        ...
        handler: () => {
          ...
          this.openCameraOrGallery('gallery');
        }
      }, {
  ...
  async openCameraOrGallery(type:'gallery') {
      
    if (type === 'gallery') {
      // Código para abrir el explorador de archivos (galería)
      ...
      input.onchange = (event: any) => {
        ...
        if (file) {
          ...
          reader.onload = (e: any) => {
            ...
            this.uploadImageToFirebase(base64Image);
          };
          reader.readAsDataURL(file); // Convertimos la imagen a Base64
        }
      };
      input.click(); // Simula el clic en el input para abrir el explorador de archivos
    }
  }
  
  // Función para subir la imagen a Firebase
  uploadImageToFirebase(base64Image: string) {
    this.util.show(); // Muestra el cargador
    ...
    firebase.storage().ref().child(localStorage.getItem('uid')).child(btoa(id) + '.jpg')
      ...
      .then((snapshot) => {
        ...
        snapshot.ref.getDownloadURL().then((url) => {
          ...
          this.coverImage = url; // Guarda la URL de la imagen
        });
      }, error => {
        this.util.hide();
        console.log('Error al subir la imagen:', error);
        this.util.errorToast('Algo salió mal');
      });
  }
